...
public class PrintfFormat {
    ...
    public PrintfFormat(Locale locale, String fmtArg) throws IllegalArgumentException {
        ...
        for (int i = 0; i < vFmt.size(); i++) {
            ConversionSpecification cs = (ConversionSpecification) vFmt.get(i);
            if (cs.getConversionCharacter() != '\0') {
                formatCharacters.add(cs.getConversionCharacter() + "");
            }
        }
    }
    ...
    /**
     * Returns the array of format specifiers (s, i, e, d, ...) in the format String
     * in the order they appear
     * @return
     */
    public char[] getFormatSpecifiers() {
        char[] formatSpecifiers = new char[formatCharacters.size()];
        for (int i = 0; i < formatCharacters.size(); i++) {
            String formatSpecifier = (String) formatCharacters.get(i);
            formatSpecifiers[i] = formatSpecifier.charAt(0);
        }
        return formatSpecifiers;
    }

    /**
     * Format an array of objects represented as Strings.
     * This method examines the format specifiers and attempts a
     * conversion of the corresponding array member to the proper
     * wrapper class.
     * e.g. format: "%f  %s %i"
     *      input : {"11.2", "by", "100"}
     *        would convert the input to
     *      {new Double("11.2"), "by", new Integer("100")}
     * If a conversion fails, a NumberFormatException will be thrown.
     * @param s The array of String reprsentations of objects to format.
     * @return  The formatted String.
     */
    public String cprintf(String[] s) {
        if (s == null) {
            return sprintf(s);
        }
        char[] formatSpecifiers = getFormatSpecifiers();
        Object[] converted = new Object[s.length];
        for (int i = 0; i < s.length; i++) {
            char formatSpecifier = formatSpecifiers[ (i < formatSpecifiers.length ? i : formatSpecifiers.length - 1)];
            switch (formatSpecifier) {
                case ('d'): //All falls through to the Integer conversion
                case ('c'):
                case ('o'):
                case ('x'):
                case ('X'):
                case ('i'):
                    ...
                    converted[i] = new Integer(d.intValue());
                    break;
                case ('e'): //All falls through to the Double (floating point) conversion
                case ('E'):
                case ('g'):
                case ('G'):
                case ('f'):
                    converted[i] = new Double(s[i]);
                    break;
                case ('s'): //Falls through to the default/String
                default:
                    converted[i] = s[i];
                    break;
            }
        }
        return sprintf(converted);
    }
    ...
    /** Vector of control strings and format literals. */
    private Vector formatCharacters = new Vector();
    ...
    public static void main(String[] a) {
        PrintfFormat pf = new PrintfFormat("Please use this %s stuff %i times");
        System.out.println(pf.getFormatSpecifiers().length);
        String[] values = new String[] {
            new String("crappy"),
            new String("1-0")
        };
        System.out.println(pf.cprintf(values));
    }
}
